...
module Primer
  module Alpha
    class TreeViewTest < Minitest::Test
      ...
      def test_allows_multi_select_for_async_sub_trees
        render_inline(Primer::Alpha::TreeView.new) do |tree|
        ...
        assert_selector("tree-view-include-fragment[src='/foobar']")
      end
      ...
      def test_disallows_select_variants_when_anchor_variant_is_used
        error = assert_raises(ArgumentError) do
          render_inline(Primer::Alpha::TreeView.new(node_variant: :anchor)) do |tree|
            tree.with_sub_tree(label: "src", select_variant: :multiple) do |sub_tree|
              sub_tree.with_leaf(label: "button.rb")
            end
          end
      ...
      def test_disallows_select_variants_when_button_variant_is_used
        error = assert_raises(ArgumentError) do
          render_inline(Primer::Alpha::TreeView.new(node_variant: :button)) do |tree|
            tree.with_sub_tree(label: "src", select_variant: :multiple) do |sub_tree|
              sub_tree.with_leaf(label: "button.rb")
            end
          end
      ...
      def test_falls_back_to_div_variant_when_invalid_variant_given
        without_fetch_or_fallback_raises do
          render_inline(Primer::Alpha::TreeView.new(node_variant: :foo)) do |tree|
